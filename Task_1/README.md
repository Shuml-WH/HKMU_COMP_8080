...
- `mental_data.py` (**Model**): Contains the data structures and logic. It defines the `Person`, `Patient` and  `MentalData` classes which handles loading, filtering, and updating the dataset.
- `dashMentalLayout.py` (**View & Controller**): Defines the frontend layout using Dash HTML/Core components and contains the callback functions that map user interactions.
- `data/mental_health_diagnosis_treatment_.csv`: The underlying dataset from Kaggle.
